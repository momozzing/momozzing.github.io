...
Task-Oriented Dialog system의 구성 요소중에 DST 단계가 있다. DST를 위한 논문.
...
2. System inform memory -> System의 inform을 Tracking하는 system inform memory에서 값을 복사해서 사용할 수 있음.
...
이 방법론은 System inform memory와  DS Memory(이전에 본 슬롯)에 대한 두 개의 메모리를 즉석에서 생성하고 유지해서 위의 2, 3번의 값을 채울 수 있다. 
...
$a_t^{inform}$ = system inform memory와 $a_t^{ds}$ = DS(domain slot) memory을 현재 상황정보를 보존할수 있는 Auxiliary input으로 사용한다.
### **Context Encoder**
...
$C = {\{none, dontcare, span, inform, refer}\}$
...
system inform memory = system에서 원하는 slot이 나왔을경우 이를 복사해서 사용 
...
system inform memory, DS memory 기반으로 보조 입력을 생성한다. 

$a_t^{inform}$은 system inform memory를 언제 copy해서 사용했는지 
